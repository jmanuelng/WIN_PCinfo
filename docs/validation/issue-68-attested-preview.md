...
| Warning | The bundle includes `LIMITED-TRUST.md` and verification compares that page to the policy-derived bytes. Generated-application `-Workflow VerifyAttestation` emits `win-pcinfo.limited-trust-warning` as the first record. Ordinary Help, About, Verify, and Assessment launches do not inherit the warning. |
| Verifier | `tests/AttestedPreviewApplication.Tests.ps1` verifies the clean bundle, then mutates the application, resource, manifest, checksum, provenance, source revision, dependency inventory, SBOM, and warning page. Each class returns `NotStarted` with a typed `ATTESTATION.*` reason. Missing inputs, Trusted labels, a false unsigned flag, convenience reasons, and a claimed Stable signing gate are rejected. Preparation fixtures cannot override the result. |
