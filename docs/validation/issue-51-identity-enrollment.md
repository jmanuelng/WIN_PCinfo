...
| Structured sources | Two independently bounded standard-user attempts use `NetGetJoinInformation`, `NetGetAadJoinInformation` (`S_FALSE` means absent; `S_OK` requires a non-null structure), Terminal Services, and local LSA logon-session APIs; each has its own envelope and output bound, with no localized command output, domain-capable name lookup, or tenant authentication. |
