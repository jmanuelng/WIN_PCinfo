...
| Offline admission | `tests/AzureValidationAdmission.Tests.ps1` admits synthetic one-client and four-client plans, admits a non-claiming diagnostic VM, and rejects a fifth client, over-budget lifetime, missing cleanup reserve, repository path, public path, UNC path, missing marker, floating provider version, live plan request, missing tags, public IP, transitive peering, Basic NAT IP, Premium disk, claiming non-Trusted-Launch Windows 11, and oversized SKU. |
