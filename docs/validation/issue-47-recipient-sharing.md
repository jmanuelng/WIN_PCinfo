...
- `RecipientSelection.Tests.ps1` drives a synthetic profile through the generated application's request and Preparation Summary, proves the fingerprint-confirmed zero-or-one selection, proves the profile path is not emitted, and proves approval cannot override a mismatch.
- `ProtectedPackageRecipient.Tests.ps1` proves zero-recipient local access, one-recipient RSA-OAEP-SHA-256 wrapping alongside DPAPI, identity-free envelope metadata, expired historical opening with the matching key, and missing-key rejection without plaintext.
- `RestrictedReportExport.Tests.ps1` proves warning refusal, interrupted-write cleanup, HTML-only output, the permanent Restricted banner, non-public classification, and all Result-sharing Guidance topics.
- `RecipientSharingApplication.Tests.ps1` invokes all eleven closed scenarios through the generated application and verifies one sanitized result, one matching terminal record, and zero validation residue.
...
The generated scenarios are `TpmBackedSetup`, `SoftwareFallbackSetup`, `ProfileValidation`, `WrongFingerprint`, `ExpiredAdmission`, `HistoricalOpening`, `MissingKey`, `ZeroRecipient`, `OneRecipient`, `InterruptedExport`, and `WarningDeclined`. They use ephemeral in-memory RSA keys and test-owned local files only. They do not create a real certificate-store identity or exercise a real recipient.
