...
| Decision engine | `tests/PreviewPublication.Tests.ps1` evaluates complete signed and attested packs plus missing approval, mismatched approval, denied qualification, wrong-candidate, privacy-unsafe, silent-replacement, GitHub-auth, immutable-tag, and download-mismatch requests. Missing approval previews only. A product failure cannot be averaged or waived. |
