...
| --- | --- |
| Deterministic report shell | `tests/ComprehensiveReport.Tests.ps1` renders the same synthetic record twice for `Completed`, `CompletedWithGaps`, `Cancelled`, `TimedOut`, `IntegrityFailed`, and `CleanupIncomplete` outcomes and verifies byte-identical HTML, ordered executive-summary headings, scripting-free offline markup, visible keyboard-focus rules, and print CSS. |
| Generated application seam | `tests/ComprehensiveReportApplication.Tests.ps1` runs the generated artifact through `DirectOutbound`, `LocalOnly`, and `Unicode` full-profile scenarios and verifies one sanitized `win-pcinfo.comprehensive-report-validation` record, one Completion Summary, one terminal record, deterministic report output, package-manifest consistency, and cleanup. |
| Package and summary agreement | The generated seam proves that the report, manifest completeness, Completion Summary assessment fields, protected-package verification, cleanup state, and terminal outcome stay aligned without exposing the package path or inner contents. |
...
| Unicode and locale safety | The Unicode scenario proves that multilingual report content survives the UTF-8 HTML path without changing the deterministic contract or widening localized parsing. |
