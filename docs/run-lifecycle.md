...
Issue #43 adds a separate generated privilege fixture seam before this finalizer exists. It proves one frozen Administrator phase and then permits the safe synthetic standard-user collector only after privilege completion or denial. Its terminal still fails package integrity rather than claiming completion. See [Frozen Administrator plan](privileged-plan.md).
