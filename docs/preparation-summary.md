...
- the Certificate Trust contract: six recipient-relevant purposes, exact read-only stores and EKUs, separate validity/chain/trust/key-protection states, Restricted fingerprints, offline chain limits, finite candidate/deadline bounds, and no private-key access or trust change.
- the local-network-topology contract: adapters, Windows connection profiles, IP configuration, active routes, configured resolvers and proxy, VPN/security registrations, and bounded existing local connections; exact offline sources; eight-entry scope bounds; and no packet capture, credential collection, health inference, or network change.
- the Microsoft Connectivity contract: catalog version, three exact generic DNS names/HTTPS URIs, port 443, `HEAD` with no body or redirects, separate DNS/TCP/TLS/chain/negotiation/proxy/HTTP/enrollment-DNS scopes, finite deadlines and evidence bounds, and a zero-outbound Local Only branch.
- the software-inventory contract: explicit machine and verified Assessment User 32-bit/64-bit uninstall views, inventory-only MSI and Windows package identities, 64-entry per-scope and 128-entry aggregate bounds, a ten-second standard-user attempt, offline/no-write behavior, Restricted identities, and no `Win32_Product`, repair, consistency check, profile loading, binary inspection, or license collection.
...
The post-approval Privileged Collection Plan is explained in [Privileged Collection Plan](privileged-collection-plan.md). Purpose-bound Certificate Trust is frozen separately as a standard-user, offline, read-only operation; it does not add a UAC interaction. The plan's one LocalSystem operation is further reduced to the separate [SYSTEM Collection Sub-plan](system-collection-sub-plan.md). These synthetic fixture families are validation-only and do not change the ordinary Preparation prerequisites.
