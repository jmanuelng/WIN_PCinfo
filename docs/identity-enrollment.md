...
All three are offline and bounded to one five-second attempt. The two standard-user collectors run as separate Microsoft-signed PowerShell children, each assigned to its own kill-on-close Job Object before its first instruction, so each Collector Result Envelope corresponds to one independently governed attempt and its own output limit. Timeout includes bounded termination and verified tree absence. They cannot prompt, download, install, self-elevate, launch a caller command, or write Windows state. An elevated or SYSTEM coordinator receives explicit gaps instead of running the standard-user sources. The SYSTEM worker cannot receive Assessment User Context, package authority, credentials, tenant values, or arbitrary evidence. Any collector or rule worker whose absence cannot be proved ends the run `CleanupIncomplete`, prevents packaging, and makes terminal cleanup verification false.
