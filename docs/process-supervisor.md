...
WIN-PCInfo uses one Process Supervisor whenever an Approved Collector Executable must run outside the main PowerShell process. It supervises the original synthetic safety probe, the historical eight-field Device Readiness collector, and the versioned 17-field device-context collector. These slices still do not by themselves create a Preview/Supported claim.
...
The exported interface accepts only a release-defined operation ID and, when needed, a cancellation token. The hidden device-context validation parameter accepts only 18 release-owned scenario names and cannot carry evidence or commands:
...
- a five-second operation deadline, per-operation stdout bounds (4 KiB for the original probe, 8 KiB for historical Device Readiness, and 16 KiB for device context), a 4 KiB stderr bound, and a 750 ms cooperative-cancellation grace interval;
...
Only an approved success payload is decoded as strict UTF-8 and checked against its fixed JSON shape. Historical Device Readiness accepts exactly eight bounded properties. Device context accepts exactly 19 transport properties—bounded source values plus three source-availability controls—or the one-property unavailable shape. Two additional record fields are derived only after the source observations pass contract validation. A key-like property name is rejected before `PrivatePayload` admission and produces only a typed prohibited-material marker. Raw pipe text is never copied into progress or diagnostics, and it is not hashed: unexpected Prohibited Secret Material must not gain a second representation through a digest. Retained buffers are zeroed after normalization or failure handling as best-effort memory hygiene, without making a forensic secure-erasure claim.

## Files and other residue

The supervisor creates no script file or working directory. The reviewed source remains commented in the repository; its deterministic compact representation is digest-verified and encoded directly into the fixed PowerShell argument, closing the validate-then-execute file-replacement window while staying below the Windows launch bound. The active `PSHOME` is validated as the working directory and treated as non-writing. The run-unique cancellation event disappears when the last owned handle closes. No scheduled task, service, persistent process, marker, log, or lock is created. Every success, rejection, overflow, timeout, cancellation, child-process, and incompatibility fixture verifies the owned process tree and named event absent before return.
