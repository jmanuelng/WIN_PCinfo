...
After approval, Preparation carries the exact admitted public-certificate bytes in its private execution facts. Packaging accepts only that closed approved-recipient object, rechecks its fingerprint and admission validity, keeps normal Local Package Protector access, and adds one RSA-OAEP-SHA-256 wrap of the same fresh AES content key. It does not accept a late arbitrary recipient-certificate parameter. The outer envelope exposes neither the profile, certificate, fingerprint, label, subject, issuer, user, device, nor organization. OAEP randomness also prevents the wrapped value from becoming a stable certificate identifier.
...
A zero-recipient package is DPAPI-only. Its Completion Summary therefore marks recipient private transfer unavailable: copying it does not give another person access. It remains openable only in the initiating Windows user/device protection context.
...
Every completed normal Device Readiness path and generated recipient-validation path emits a Completion Summary. It records package verification separately from the closed package-availability state (`Available`, `VerifiedAbsent`, or `Uncertain`), so validation-only runs can prove packaging and cleanup without claiming that their removed synthetic package remains accessible or transferable. Uncertain cleanup residue prohibits transfer and retains operator recovery/deletion responsibility. `ResultSharingGuidance` covers:
...
- operator-only deletion responsibility for zero recipients, or operator-and-authorized-recipient responsibility when one recipient was actually selected; and
