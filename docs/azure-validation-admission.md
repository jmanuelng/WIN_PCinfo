...
This slice does not apply a round, recount live Azure VMs, tear down residue, or publish release evidence. The controller that can run one to four admitted clients, reject a fifth, and recover to Zero Round Residue is documented in [One fresh Azure validation round](azure-validation-round.md). Later tickets must keep the same privacy boundary.
