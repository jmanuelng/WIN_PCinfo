...
function New-ControlledCapture { [pscustomobject]@{RawData=$null;ChainState=$(if($script:caseStatus -eq -1){'Invalid'}else{'Unavailable'})} }
...
function New-ControlledHttpClient {
    ...
    $client|Add-Member ScriptMethod SendAsync {
        ...
        if($script:caseStatus -lt 0){throw [Security.Authentication.AuthenticationException]::new('Synthetic TLS rejection')}
...
foreach($case in @(
    ...
    @{status=-1;expected='CertificateChainInvalid';headers=3},
    @{status=-2;expected='TlsAuthenticationFailed';headers=3},
    ...
    Assert-Equal $true ($null -eq $result.leafSha256) 'an uncaptured HTTP certificate remains null rather than an invalid empty fingerprint'
    Assert-Equal 1 $script:sends 'redirect/authentication/failure never initiates a second request'
    Assert-Equal $(if($case.status -le 0){2}else{3}) $script:disposed 'all owned response and transport objects are disposed'
