...
The portable package is a precursor identity, not a signed release. See [Portable distribution and first-run](docs/portable-distribution.md) to extract it, verify every governing resource, and use the Windows PowerShell helper. The package never installs PowerShell. The governed [Signing Boundary](docs/signing-boundary.md) can sign a frozen eligible generated candidate after release gates and human digest confirmation, then finalize and smoke-run the distinct signed distributable; that synthetic session cannot publish a Trusted release. The governed [Attested Preview](docs/attested-preview.md) fallback binds that unchanged zip only when Artifact Signing is not operational or during a verified service incident; it remains unsigned, limited-trust, and unable to satisfy the Stable signing gate.
...
To understand the offline network slice—local adapters, profiles, addressing, routes, configured resolvers/proxy, VPN and security registrations, existing local connections, privacy boundaries, and proof that Local Only makes zero assessment network requests—read [Local network topology and Local Only](docs/network-topology-and-local-only.md).

To understand the optional outbound slice—its exact generic Microsoft catalog, separate DNS/TCP/TLS/chain/proxy/HTTP evidence, TLS-inspection semantics, privacy boundaries, tenant-specific limitations, and zero-outbound Local Only branch—read [Microsoft service connectivity and enrollment discovery](docs/microsoft-connectivity-and-enrollment-dns.md).
...
To understand the maintainer-only controller that walks one to four private Windows 11 clients through synthetic VM Agent control, exclusive-lease admission, cancellation and expiry recovery, and Zero Round Residue, read [One fresh Azure validation round](docs/azure-validation-round.md). Live create stays `NotStarted` on this host. It is a controller or DEV tracer and does not deliver a Preview or Supported claim.
