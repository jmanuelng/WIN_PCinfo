...
function Invoke-BoundedNetworkTopologySnapshot {
    ...
    # Output remains untrusted PSSerializer bytes until the coordinator's exact
    ...
    try{
        ...
        [pscustomobject]@{succeeded=$true;payload=[Management.Automation.PSSerializer]::Deserialize($xml);reasonCode='';startedAt=$started;completedAt=[DateTimeOffset]::UtcNow;privateProcessId=[int]$native.ProcessId}
