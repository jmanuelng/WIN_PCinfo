...
function ConvertTo-NetworkTopologyAttemptPayload {
    ...
    if(-not $proxyValid){$scope=@($states|Where-Object scopeId -eq 'scope:network.proxy')[0];if($scope.state -in @('Complete','Partial')){$scope.state='Malformed';$scope.reasonCode='NETWORK.SOURCE_VALUE_MALFORMED'}}
    [pscustomobject][ordered]@{
        sourceLocale=$Payload.sourceLocale;assessmentUserContextVerified=$Payload.assessmentUserContextVerified
        processRelationship=$Payload.processRelationship;networkBehavior=$Payload.networkBehavior
    ...
    [Net.NetworkInformation.NetworkInterface]::GetAllNetworkInterfaces()|ForEach-Object {
        try{
            ...
            if($null -eq $index -or -not (Valid-Text $_.Name 256) -or -not (Valid-Text $_.Description 512)){Set-Scope $scopes 'scope:network.adapters' 'Partial' 'NETWORK.ADAPTER_MALFORMED'}else{
                ...
                $hardware=if($_.NetworkInterfaceType -in @([Net.NetworkInformation.NetworkInterfaceType]::Loopback,[Net.NetworkInformation.NetworkInterfaceType]::Tunnel)){$false}else{$null}
...
function Invoke-BoundedNetworkTopologySnapshot {
    ...
    try{
        ...
        $native=[WinPCInfo.ProcessSupervisor.NativeRunner]::Run($executable,@('-NoLogo','-NoProfile','-NonInteractive','-EncodedCommand',$encoded),$PSHOME,$environment,$activeMilliseconds,[int]$collector.resultMaximumUtf8Bytes,4096,(Get-AssessmentCancellationToken),$event,1,$terminationMilliseconds,$false)
...
function Add-NetworkTopologyEvidenceRecord {
    ...
    $definitions=@(
        ...
        @{scope='scope:network.connection-profiles';collection='profiles';kind='Interface';prefix='profile';fields=@(@('interfaceIndex','field:network.profile.interface-index'),@('name','field:network.profile.name'),@('category','field:network.profile.category'),@('connectivity','field:network.profile.connectivity'))},
        ...
        @{scope='scope:network.routes';collection='routes';kind='Interface';prefix='route';fields=@(@('interfaceIndex','field:network.route.interface-index'),@('addressFamily','field:network.route.address-family'),@('destinationPrefix','field:network.route.destination-prefix'),@('nextHop','field:network.route.next-hop'),@('metric','field:network.route.metric'))},
        @{scope='scope:network.resolvers';collection='resolvers';kind='Interface';prefix='resolver';fields=@(@('interfaceIndex','field:network.resolver.interface-index'),@('addressFamily','field:network.resolver.address-family'),@('addressesText','field:network.resolver.addresses'))},
