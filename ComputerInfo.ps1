...
Function Get-AADAccounts {
    ...
    $UPNs = $UserLines | ForEach-Object {
        ($_ -split ",")[1].Trim().Substring(6)  # Extracts the UPN value from the account record
    }
...
# Verify administrative priviliges. Script has to be executed with Administrative priviliges.
...
# Verify and/or install Microsoft.PowerShell.Management module.
...
# Collect DSReg status info
# File name and locations to store gathered info, the "output file".
$PC_folder = "Config"
...
# Heading and Footer to distinguish this section in Output File.
...
# Get basic Computer information: Name, owner, domain, memory, manufacturer and model from CimInstance
$PC_basicinfo = Get-CimInstance -ClassName Win32_ComputerSystem
# Add heading to Log content

# Write CiMInstance results to output file.
# Writing result to file after execution in case user cancels before ending execution. 
...
# Write SystemInfo results to output file.
# Writing result to file after execution in case user cancels before ending execution. 
...
# Write Win32_ComputerSystemProduct results to output file.
# Writing result to file after execution in case user cancels before ending execution. 
...
# Write Win32_ComputerSystem results to output file.
# Writing result to file after execution in case user cancels before ending execution. 
...
# Get path to slmgr.vbs (Software Licensing Management Tool) script, and then execute script.
# used with /dlv to display license information for the installed active Windows.
...
# Write Software Licensing Management Tool results to output file.
# Writing result to file after execution in case user cancels before ending execution. 
...
# Get Windows keys detail information. This includes Firmware OEM License.
...
# Get Windows Firmware Embedded Activation Key (OEM Windows licenses), if blank, system does not have OEM license
...
# Write License keys detailed results to output file.
# Writing result to file after execution in case user cancels before ending execution.
...
# Banner and heading to distinguish this section in Output File.
$logTitle = "BIOS and TPM Information"

# Get BIOS and TPM information. This is to understand if the system has TPM, and what version.
$PC_biostpm = Get-BiosTpm

# It then adds BIOS and TPM info to file.
...
# Get info for applied policies on device, it will list Local and Domain policies applied.
$PC_GpoInfo = Get-GPOInventory | Format-Table

# It then adds Policies information to file.
...
# Get installed printers information.
...
# get profile information for connected netwrok adapter using PS command
$NET_connection = Get-NetConnectionProfile | Format-List -Property * | Out-String

# get Lan and WLan profiles with netsh
# netsh is used to get a list of all profiles, stores the list in a TXT file, puts in a folder
...
# Notify user of report fil creation.
...
# Notify about Software Inventory report.
$msg = "`...Writing Software Inventory to CSV file."
Write-Host $msg -ForegroundColor White

# Create Software Inventory report, show it in GridView.
...
=============================================================================
#>
# If laptop, create battery report
If (fnDetectLaptop) 
    { 
